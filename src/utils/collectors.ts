...
class ReactionHandler extends EventEmitter {
	...
	constructor(
		...
		filter: (userID: string, emoji: Emoji) => boolean,
		options = { time: 1e4, maxMatches: 1 }
	) {
		super();
		...
		this.ended = false;
		this.collected = [];
		this.listener = (message, emoji, userID) =>
			this.verify(message, emoji, userID);
...
export default class Collector {
	...
	public awaitReactions(
		...
		return new Promise((resolve: (value: Reaction[]) => void) =>
